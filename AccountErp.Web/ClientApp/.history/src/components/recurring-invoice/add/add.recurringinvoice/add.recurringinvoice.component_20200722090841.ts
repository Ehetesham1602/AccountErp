...
import { InvoiceAddModel, CustomerDetailModel, ItemListItemModel, AttachmentAddModel } from 'src/models';
...
@Component({
  selector: 'app-add.recurringinvoice',
  templateUrl: './add.recurringinvoice.component.html',
  styleUrls: ['./add.recurringinvoice.component.css']
})
export class AddRecurringinvoiceComponent implements OnInit {

  @BlockUI('container-blockui') blockUI: NgBlockUI;
  modalReference: any;
  disableCustomerId = false;
  model: RecurringInvoiceAddModel = new RecurringInvoiceAddModel();
 ...
 // customers: Array<SelectListItemModel> = new Array<SelectListItemModel>();
  ...
  config2 = {displayKey:"value",search:true,limitTo:10,height: 'auto',placeholder:'Select Due',customComparator: ()=>{},moreText: 'more',noResultsFound: 'No results found!',searchPlaceholder:'Search',searchOnKey: 'value',clearOnSelection: false,inputDirection: 'ltr',}
  selectedCustomer;
  ...
  setDefaultDate(){
    this.invPaymentdue=this.recurringOptions.dueOptions;
    this.selectedDue=this.invPaymentdue.find(x=> x.keyInt==1);
  }
  ...
  submit() {
      ...
      this.recinvoiceService.add(this.model).subscribe(
          () => {
              this.blockUI.stop();
              setTimeout(() => {
...
getNewInvoiceNumber() {
  this.blockUI.start();
  this.recinvoiceService.getNewInvoiceNumber()
      .subscribe((data) => {
          debugger;
          this.blockUI.stop();
